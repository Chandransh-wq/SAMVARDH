...
const Subject: React.FC<SubjectProps> = ({ darkMode }) => {
  const { id } = useParams();
  const notebook = notebookData.find(nb => nb.id === id);
      ...
      <div className="w-1/2 text-left pl-12">
        ...
        <div>
          <span className='text-3xl tracking-wider font-semibold flex items-center gap-3'>
            <div className='h-5 w-5 rounded-full' style={{ backgroundColor: darkMode ? notebook?.color : hexToRgba(notebook ? notebook.color : "#ccc", 0.7) }}></div>
            {notebook ? notebook.title : "Notebook not found"}
          </span>
        ...
        <div className='mt-5'> 
            ...
            <div className='mt-5 flex flex-col gap-15'>
                {notebook ? notebook.subjects.map((subject, idx) => {
                    // Pick first matching tag or default to 'Other'
                    ...
                    return (
                        ...
                        >
                            <div className='w-1/4 p-2'>
                                <div
                                  dangerouslySetInnerHTML={{ __html: basic[tagKey as keyof typeof basic] }}
                                />
                            </div>
                            <div className='flex flex-col w-3/4'>
                                ...
                                <div className='flex gap-4 my-1'>
                                    {subject.tags.map((tag, idx) => (
                                        <div key={idx} className='bg-blue-100 p-2 px-3 rounded-lg text-black'>
                                            {tag}
                                        </div>
                                    ))}
                                </div>
                                <div className='flex items-center justify-between pr-6'>
                                    <span className='mt-3'>Created At :&nbsp;
                                        <span className='font-semibold text-lg'>
                                            {subject ? new Date(subject.createdAt).toLocaleDateString("en-UK", options3) : "-"}
                                        </span>
                                    </span>
                                </div>
                            </div>
      ...
      >
        ...
        <div className="flex items-center p-4 px-4 justify-center gap-4 text-gray-400 font-medium w-full relative mt-6">
          <button
            ...
            className="p-2 rounded-full bg-zinc-400 text-white hover:bg-gray-400 dark:hover:bg-gray-700 disabled:opacity-40 absolute left-0"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>

          <motion.div
            className="flex flex-nowrap gap-6 overflow-hidden mx-12"
            key={monthWindowStart}
          ...
          <button
            ...
            disabled={monthWindowStart >= months.length - 5}
            className="p-2 rounded-full bg-zinc-400 text-white hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-40 absolute right-0"
          >
        ...
        <div
          ref={dayContainerRef}
          className="flex gap-3 mt-6 overflow-x-auto my-scrollbar py-2"
        >
          {daysArray.map((day) => {
            ...
            return (
              <motion.button
                ...
                className={`flex flex-col items-center justify-center min-w-[4rem] w-14 h-20 rounded-2xl shadow-sm transition ${
                  isActive
                    ? "bg-orange-500 text-white active-day"
                    : darkMode
        ...
        <div className='text-left text-lg tracking-wide font-semibold mb-4'>
          Topics for {notebook?.subjects[selected]?.title} :
        </div>
...
<div className='flex flex-col gap-5'>
  {carsoulSelected === 1 && notebook?.subjects[selected]?.topics && (
    (() => {
      const selectedDate = new Date(today.getFullYear(), selectedMonth, selectedDay);
      const filteredTopics = notebook.subjects[selected].topics.filter(topic => {
        if (!topic.dueDate) return true; // include topics with no dueDate
        const topicDate = new Date(topic.dueDate);
        return topicDate >= selectedDate; // only topics on/after selected day
      });
  ...
  {carsoulSelected === 2 && notebook?.subjects[selected]?.topics && (
    notebook.subjects[selected].topics.length === 0 ? (
      <div className='w-full text-center py-10 text-gray-500 text-lg'>
        No Topics Due for now
      </div>
    ) : (
      notebook.subjects[selected].topics.map((topic, idx) => (
        <div
          key={idx}
          className={`w-full rounded-2xl ${darkMode ? "bg-zinc-950 hover:bg-zinc-900" : "bg-zinc-50 hover:bg-zinc-100"} group p-3 shadow-md flex items-center gap-4 cursor-pointer transition-all duration-200`}
          onClick={() => navigate(`/subject/${topic.id}`)}
  ...
  {/* fallback if no topics exist at all */}
  {!notebook?.subjects[selected]?.topics && (
    <div className='w-full text-center py-10 text-gray-500 text-lg'>
      No Topics Due for now
    </div>
  )}
